"""
Geeni device adapter (Tuya protocol).

This adapter uses the tinytuya library to control Geeni smart lights.
Geeni devices use the Tuya protocol, so this adapter can also work with
other Tuya-based devices.
"""
...
class GeeniDevice(SmartDevice):
    """
    Adapter for Geeni smart lights (Tuya protocol).
    
    Supports Geeni smart bulbs and other Tuya-based devices.
    ...
    def __init__(
        ...
        """
        Initialize Geeni device adapter.
    ...
    async def _get_device(self) -> BulbDevice:
        ...
        try:
            ...
            raise DeviceConnectionError(
                f"Failed to connect to Geeni device at {self._device_ip}: {str(e)}"
            ) from e
    
    async def turn_on(self) -> None:
        """Turn the Geeni device on."""
        try:
            ...
            raise DeviceError(f"Failed to turn on Geeni device: {str(e)}") from e
    
    async def turn_off(self) -> None:
        """Turn the Geeni device off."""
        try:
            device = await self._get_device()
            device.turn_off()
        except DeviceConnectionError:
            ...
            raise DeviceError(f"Failed to turn off Geeni device: {str(e)}") from e
    
    async def is_on(self) -> bool:
        """Check if the Geeni device is currently on."""
        try:
            device = await self._get_device()
            ...
            raise DeviceError(f"Failed to get Geeni device state: {str(e)}") from e
    
    async def get_status(self) -> Dict[str, Any]:
        """
        Get the current status of the Geeni device.
        ...
        try:
            ...
            raise DeviceError(f"Failed to get Geeni device status: {str(e)}") from e
    
    async def set_brightness(self, level: int) -> None:
        """
        Set the brightness level of the Geeni device (0-100).
    ...
    async def set_color(self, color: str) -> None:
        """
        Set the color of the Geeni device.
